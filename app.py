...
# Reorder columns to put EPS % Change right after EPS Next Year
                ...
                for col in percentage_cols:
                    ...
                    if col == "Debt To Equity TTM":
                        ...
                        pos_vals = numeric_col_nonnull[numeric_col_nonnull > 0]
                        neg_vals = numeric_col_nonnull[numeric_col_nonnull < 0]

                         # For Debt to Equity, lower is better
                        top_idxs = pos_vals.nsmallest(2).index if not pos_vals.empty else []
                        bottom_idxs = neg_vals.nsmallest(2).index if not neg_vals.empty else []
                    else:
...
# Display the table
    ...
    '3 Year Free Cashflow CAGR': 0.15,
            ...
            lambda x: 1 - ((x - recommend_df[col].min()) / (recommend_df[col].max() - recommend_df[col].min()))
                ...
                def fetch_transcript_latest(ticker):
                    for year, quarter in [(2025, 1), (2024, 4)]:
                        try:
                            ...
                            if data and "content" in data[0]:
                                return data[0]["content"], year, quarter
                        except:
                            continue
                    raise ValueError(f"No recent transcript found for {ticker}.")

                def summarize_earnings(content, ticker, quarter, year):
                    prompt = f"""
                You are a financial research assistant. Create a detailed summary of the {ticker} Q{quarter} {year} earnings call using these sections:

                 1. 📊 Key Financial Results – Provide a high-level summary of EPS, revenue, and major business segment performance.
                 2. 🗣️ Executive Commentary – Highlight tone, strategy, platform updates, and leadership remarks.
                 3. 🔭 Outlook – Summarize forward-looking statements or guidance.
                 4. 🎤 Q&A Highlights – Extract 3+ useful insights from analyst questions and responses.
                 5. 🧠 LLM-Based Sentiment Analysis – Provide a sentiment label and rationale.

                Keep the summary clear and concise. Useful to investors.
                Transcript:
                \"\"\"
                {content}
                \"\"\"
                ...
                def extract_sentiment(summary):
                    text = summary.lower()
                    match = re.search(r"sentiment(?: label)?(?: is|:)?\s*(positive|neutral|negative)", text, re.IGNORECASE)
                    if match:
                        label = match.group(1).capitalize()
                    else:
                        if any(word in text for word in ["confident", "optimistic", "strong performance", "growth", "beat expectations", "robust"]):
                            label = "Positive"
                        elif any(word in text for word in ["uncertain", "mixed", "moderate", "flat", "neutral"]):
                            label = "Neutral"
                        elif any(word in text for word in ["disappointing", "cautious", "decline", "challenging", "negative"]):
                            label = "Negative"
                        else:
                            label = "Unknown"

                    icon = {"Positive": "🟢", "Neutral": "⚪", "Negative": "🔴", "Unknown": "🔴"}.get(label, "🔴")
                    return f"{icon} {label}"
                
                st.subheader("🧾 GPT Earnings Summary for Top Stock")
                if 'top' in locals():
                    try:
                        with st.spinner(f"Fetching transcript for {top}..."):
                            transcript, year, quarter = fetch_transcript_latest(top)
                            summary_raw = summarize_earnings(transcript, top, quarter, year)
                            summary = clean_markdown(summary_raw)
                            sentiment = extract_sentiment(summary_raw)

                        st.markdown(f"### 📝 {top} Q{quarter} {year} Earnings Summary")
                        st.markdown(f"**Sentiment:** {sentiment}")

                        sections = {
                            "📊 Key Financial Results": "🗣️ Executive Commentary",
                            "🗣️ Executive Commentary": "🔭 Outlook",
                            "🔭 Outlook": "🎤 Q&A Highlights",
                            "🎤 Q&A Highlights": "🧠 LLM-Based Sentiment Analysis"
                                                                            }
                        for start, end in sections.items():
                            if start in summary:
                                content = summary.split(start)[-1].split(end)[0].strip()
                                st.markdown(f"**{start}**")
                                st.write(content.split("\n"))
                    except Exception as e:
                         st.error(f"❌ Could not generate earnings summary: {e}")
                else:
                    st.warning("Top ticker not found. Please run analysis first.")
